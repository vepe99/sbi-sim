...
from flax.linen.attention import dot_product_attention, DenseGeneral

# --- GLU helper ---
class GLULayer_theta(nn.Module):
    dim_x: int
    dim_theta_t: int
    out_dim: int
    ...
    def __call__(self, x, theta_t):
        # x-stream
        h_x = nn.Dense(self.out_dim, kernel_init=self.kernel_init)(x)
        # gating stream
        gate = nn.Dense(self.out_dim, kernel_init=self.kernel_init)(theta_t)
        gate = jax.nn.sigmoid(gate)
        # gated output
        gate = jnp.squeeze(gate, axis=-2)  # or axis that is singleton
        return h_x * gate
...
class MAB(nn.Module):
    ...
    def __call__(
        ...
        # --- Conditioning injection (theta) ---
        if theta_emb is not None:
            theta_exp = jnp.repeat(theta_emb[:, None, :], N_y, axis=1)  # (B, N_y, D_theta)
            y = GLULayer_theta(
                dim_x=y.shape[-1],
                dim_theta_t=theta_exp.shape[-1],
                out_dim=y.shape[-1],
        ...
        # --- Time embedding via GLU ---
        if t_emb is not None:
            t_q = jnp.repeat(t_emb[:, None, :], N_x, axis=1)  # (B, N_x, D_t)
            t_k = jnp.repeat(t_emb[:, None, :], N_y, axis=1)  # (B, N_y, D_t)

            # project to dim_split
            t_q_proj = DenseGeneral(features=(self.N_head, dim_split), kernel_init=kernel_init)(t_q)
            t_k_proj = DenseGeneral(features=(self.N_head, dim_split), kernel_init=kernel_init)(t_k)

            q = GLULayer(dim_x=dim_split, dim_theta_t=t_q_proj.shape[-1], out_dim=dim_split, kernel_init=kernel_init)(q, t_q_proj)
            k = GLULayer(dim_x=dim_split, dim_theta_t=t_k_proj.shape[-1], out_dim=dim_split, kernel_init=kernel_init)(k, t_k_proj)


        # --- Attention ---
        a = dot_product_attention(q, k, v)  # (B, N_x, N_head, dim_split)
        ...
        o_ff = nn.relu(DenseGeneral(self.N_dim, kernel_init=kernel_init)(o))
        o = o + o_ff
