...
class corner_plot_posterior(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        figure_list = []
        fig2, axes = plt.subplots(1, 4, figsize=(25, 5))  # Create figure and 4 axes once
        ...
        for id in self.observation_idx:
            ...
            for i in range(4):
                ax = axes[i]
                mean = jnp.mean(posterior_samples["samples"][:, i]).item()
                std = jnp.std(posterior_samples["samples"][:, i]).item()
                ax.errorbar(
                    true_theta[0, i].item(),
                    mean,
                    yerr=std,
                ...
                ax.plot([self.low[i], self.high[i]], [self.low[i], self.high[i]], 'k--', alpha=0.5, )
                ax.set_xlabel(f'True {labels[i]}')
                ax.set_ylabel(f'Predicted {labels[i]}')
                ax.set_xlim(self.low[i], self.high[i])
                ax.set_ylim(self.low[i], self.high[i])

            if self.savedir is not None:
                fig.savefig(f"{self.savedir}/pictures/corner_plot_{id}_{epoch}.png")
            
            figure_list.append(fig)

        if self.savedir is not None:
                fig2.savefig(f"{self.savedir}/pictures/true_vs_predicted_{epoch}.png")
...
class ranks(Callback):
    ...
    def __init__(self, save_every: int = 10, savedir: str = None, num_total_samples: int = 1_000, observation_idx: int = 10, batch_size: int = 1_000):
        super().__init__()
        self.save_every = save_every
        self.savedir = savedir
        self.num_total_samples = num_total_samples
