...
class C2ST_OT_AllParametersPositions(Callback):
...
class C2ST_OT_AllParametersPositions_TrueSimulation(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        print('start testing')
        observation = jnp.array(np.load(self.true_GD1_observation_path)['x']).reshape(1, 1000, 6)
        true_theta = jnp.array(np.load(self.true_GD1_observation_path)['theta'])
        print(f"true theta shape: {true_theta.shape}")
        true_theta_pos = jnp.array([11.8, 0.79, 6.4, 109.5, -254.5, -90.3])
        print(f"true theta pos shape: {true_theta_pos.shape}")
        true_theta = jnp.concatenate([true_theta, true_theta_pos], axis=-1)
        print(f"true theta shape after concat: {true_theta.shape}")
...
class C2ST_OT_LOGAllParametersPositions(Callback):
...
class C2ST_OT_LOGAllParametersPositions_TrueSimulation(Callback):
...
class C2ST_OT_AllParametersPositions_newprior(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        for id in self.observation_idx:
            ...
            posterior_samples, _ = strategy.sample(self.num_total_samples, rng, conditioning=observation,
                                                   batch_size=self.batch_size)
            posterior_samples['samples'] = posterior_samples['samples'] * train_loader.std_X + train_loader.mean_X
            predicted_theta_test_set[:, id-1, :] = posterior_samples['samples']
        print(f"Predicted theta shape: {predicted_theta_test_set.shape}")
        print(f'true_theta_test_set: {true_theta_test_set.shape}')
...
class C2ST_OT_AllParametersPositions_newprior_TrueSimulation(Callback):
...
class C2ST_OT_AllParametersPositions_uniformprior(Callback):
...
class C2ST_OT_AllParametersPositions_uniformprior_TrueSimulation(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        df = pd.DataFrame(posterior_samples['samples'], columns=self.labels)
        df = df.astype(float)
        df['$M_{NFW}$'] = 10**df['$M_{NFW}$']
        df['$M_{plummer}$'] = 10**df['$M_{plummer}$']
        df['$M_{MN}$'] = 10**df['$M_{MN}$']
        # Convert to float and handle any non-numeric columns
        numeric_df = df.select_dtypes(include=[np.number])
        if numeric_df.empty:
            # If no numeric columns, convert all to float
...
class C2ST_OT_AllParametersPositions_uniformprior_uniformnorm(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        for id in self.observation_idx:
            ...
            posterior_samples, _ = strategy.sample(self.num_total_samples, rng, conditioning=observation,
                                                   batch_size=self.batch_size)
            # Current transformation back to original space (from [-1,1] to uniform bounds)
            posterior_samples['samples'] = 0.5 * (posterior_samples['samples'] + 1) * (self.high-self.low) + self.low
            predicted_theta_test_set[:, id-1, :] = posterior_samples['samples']
        print(f"Predicted theta shape: {predicted_theta_test_set.shape}")
        print(f'true_theta_test_set: {true_theta_test_set.shape}')
...
class C2ST_OT_AllParametersPositions_uniformprior_uniformnorm_TrueSimulation(Callback):
...
class C2ST_OT_AllParametersPositions_uniformprior_log(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        for id in self.observation_idx:
            ...
            posterior_samples, _ = strategy.sample(self.num_total_samples, rng, conditioning=observation,
                                                   batch_size=self.batch_size)
            posterior_samples['samples'] = posterior_samples['samples'] * train_loader.std_X + train_loader.mean_X
            posterior_samples['samples'] = posterior_samples['samples'].at[:, [0, 1, 2, 4, 6]].set(10**posterior_samples['samples'][:, [0, 1, 2, 4, 6]])
            predicted_theta_test_set[:, id-1, :] = posterior_samples['samples']
        print(f"Predicted theta shape: {predicted_theta_test_set.shape}")
        print(f'true_theta_test_set: {true_theta_test_set.shape}')
...
class C2ST_OT_AllParametersPositions_uniformprior_log_TrueSimulation(Callback):
...
class C2ST_OT_AllParametersFixPositions_uniformprior(Callback):
    ...
    def __init__(self, save_every: int = 0, savedir: str = None, num_total_samples: int = 1_000, observation_idx: int = 10, batch_size: int = 1_000):
        ...
        self.observation_idx = [i for i in range(1, observation_idx)]
        self.batch_size = batch_size
        self.labels = ['$M_{NFW}$', '$r_{NFW}$', '$M_{MN}$', '$a_{MN}$', '$b_{MN}$', '$M_{PSP}$', '$r_{PSP}$']
        if self.savedir is not None:
            os.makedirs(self.savedir + '/pictures', exist_ok=True)
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        if hasattr(strategy, "scaled_model") and strategy.scaled_model is not None:
            strategy.scaled_model.batch_stats = getattr(strategy, "batch_stats", None)
        ...
        for id in self.observation_idx:
            ...
            observation, true_theta, reference_posterior = train_loader.get_observation(id)
            true_theta_test_set[id-1, :] = 10**true_theta[0, :]

            observation = jnp.array(observation).repeat(self.num_total_samples, axis=0)
            posterior_samples, _ = strategy.sample(self.num_total_samples, rng, conditioning=observation,
                                                   batch_size=self.batch_size)
            posterior_samples['samples'] = posterior_samples['samples'] * train_loader.std_X + train_loader.mean_X
            if train_loader.use_log:
                predicted_theta_test_set[:, id-1, :] = 10**posterior_samples['samples']
            else:
                predicted_theta_test_set[:, id-1, :] = posterior_samples['samples']
        print(f"Predicted theta shape: {predicted_theta_test_set.shape}")
        print(f'true_theta_test_set: {true_theta_test_set.shape}')
...
class C2ST_OT_AllParametersFixPositions_uniformprior_TrueSimulation(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        true_theta = jnp.array([4.3683325e11, 16.,  68_193_902_782.346756, 3., 0.280, 4501365375.06545, 1.9])
        ...
        posterior_samples['samples'] = posterior_samples['samples'] * train_loader.std_X + train_loader.mean_X
        if train_loader.use_log:
            posterior_samples['samples'] = 10**posterior_samples['samples']
        else:
            posterior_samples['samples'] = posterior_samples['samples']
        np.savez(f"{self.savedir}/pictures/true_observation_prediction.npz",
                 predicted_theta=posterior_samples['samples'],
                 true_theta=true_theta)
...
class C2ST_OT_AllParametersPositions_fixedtime_uniformprior(Callback):
    ...
    def __init__(self, save_every: int = 0, savedir: str = None, num_total_samples: int = 1_000, observation_idx: int = 10, batch_size: int = 1_000):
        ...
        self.observation_idx = [i for i in range(1, observation_idx)]
        self.batch_size = batch_size
        self.labels = ['$M_{NFW}$', '$r_{NFW}$', '$M_{MN}$', '$a_{MN}$', '$b_{MN}$', '$M_{PSP}$', '$r_{PSP}$', '$x$', '$y$', '$z$', '$v_x$', '$v_y$', '$v_z$']
        if self.savedir is not None:
            os.makedirs(self.savedir + '/pictures', exist_ok=True)
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        for id in self.observation_idx:
            ...
            observation, true_theta, reference_posterior = train_loader.get_observation(id)
            if train_loader.use_log:
                true_theta[:, [0, 1, 2, 4, 6]] = 10**true_theta[:, [0, 1, 2, 4, 6]]
            true_theta_test_set[id-1, :] = true_theta[0, :]
...
class C2ST_OT_AllParametersPositions_fixedtime_uniformprior_TrueSimulation(Callback):
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        true_theta = jnp.array([4.3683325e11, 16.,  68_193_902_782.346756, 3., 0.280, 4501365375.06545, 1.9,
                                11.8, 0.79, 6.4, 109.5, -254.5, -90.3])
...
class C2ST_OT_AllParametersPositions_galax_uniformprior(Callback):
    ...
    def __init__(self, save_every: int = 0, savedir: str = None, num_total_samples: int = 1_000, observation_idx: int = 10, batch_size: int = 1_000):
        ...
        self.observation_idx = [i for i in range(1, observation_idx)]
        self.batch_size = batch_size
        self.labels = ['$M_{Plummer}$', '$M_{NFW}$', '$r_{NFW}$', '$M_{MN}$', '$a_{MN}$', '$b_{MN}$', '$x$', '$y$', '$z$', '$v_x$', '$v_y$', '$v_z$']
        if self.savedir is not None:
            os.makedirs(self.savedir + '/pictures', exist_ok=True)
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        true_theta_test_set = np.zeros((len(self.observation_idx), 12))
        predicted_theta_test_set = np.zeros((self.num_total_samples, len(self.observation_idx), 12))
...
class C2ST_OT_AllParametersPositions_galax_uniformprior_TrueSimulation(Callback):
    ...
    def __init__(self, savedir: str = None, num_total_samples: int = 1_000, batch_size: int = 6):
        ...
        self.savedir = savedir
        self.num_total_samples = num_total_samples
        self.true_GD1_observation_path = '/export/data/vgiusepp/galax_data/data_varying_position_uniform_prior/sbi_sim/data/sbi-benchmarks/galax_AllParametersPosition_uniformprior/true.npz'
        self.batch_size = batch_size
        self.labels = ['$M_{Plummer}$', '$M_{NFW}$', '$r_{NFW}$', '$M_{MN}$', '$a_{MN}$', '$b_{MN}$', '$x$', '$y$', '$z$', '$v_x$', '$v_y$', '$v_z$']
        if self.savedir is not None:
            os.makedirs(self.savedir + '/pictures', exist_ok=True)
    ...
    def _call(self, logs: dict, rng: jr.PRNGKey, strategy: Strategy,
        ...
        print('start testing')
        observation = jnp.array(np.load(self.true_GD1_observation_path)['x']).reshape(1, 200, 6)
        # true_theta = jnp.array(np.load(self.true_GD1_observation_path)['theta'])
        ...
        c.add_chain(Chain(samples=numeric_df, name="Experimental Results", ), )
        c.add_truth(Truth(location={
             '$M_{Plummer}$': true_theta[0, 0],
             '$M_{NFW}$': true_theta[0, 1],
             '$r_{NFW}$': true_theta[0, 2],
             '$M_{MN}$': true_theta[0, 3],
             '$a_{MN}$': true_theta[0, 4],
             '$b_{MN}$': true_theta[0, 5],
             '$x$': true_theta[0, 6],
             '$y$': true_theta[0, 7],
             '$z$': true_theta[0, 8],
             '$v_x$': true_theta[0, 9],
