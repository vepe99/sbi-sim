from abc import ABC, abstractmethod
from math import ceil
from pathlib import Path
from typing import Iterable, Tuple

import jax
import jax.random as jr
import jax.numpy as jnp
from jax.scipy.stats import norm
import numpy as np

import pandas as pd
from math import log10

class GeneratorDataloader(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        data = pd.read_csv(file)

        return jnp.array(data.values)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(self.dataset).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.csv"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.csv"))

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.csv"))

        if self.normalize:
            true_theta = (true_theta - self.mean_X) / self.std_X
            reference_posterior = (reference_posterior - self.mean_X) / self.std_X
            observation = (observation - self.mean_y) / self.std_y

        return observation, true_theta, reference_posterior
    
    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False,):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """

        DATA_ROOT = '/export/data/vgiusepp/odisseo_data/data_fix_position/sbi-sim/data/sbi-benchmarks'
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [1000, 10000, 100000, 1000000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [1000, 10000, 100000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.csv"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.csv"))


        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)

        self.mean_y = self.y.mean(axis=0)
        self.std_y = self.y.std(axis=0)

        if self.normalize:
            print(f"Normalizing {self.dataset} data")
            self.X = (self.X - self.mean_X) / self.std_X
            self.y = (self.y - self.mean_y) / self.std_y

        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step]
                y = self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step]
            else:
                X = self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))]
                y = self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))]
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path).joinpath(dataset)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class GaussianLinear(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("gaussian_linear", *args, **kwargs)


class BernoulliGLM(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("bernoulli_glm", *args, **kwargs)


class BernoulliGLMRaw(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("bernoulli_glm_raw", *args, **kwargs)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(self.dataset).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation_raw.csv"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.csv"))

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.csv"))

        if self.normalize:
            true_theta = (true_theta - self.mean) / self.std
            reference_posterior = (reference_posterior - self.mean) / self.std

        return observation, true_theta, reference_posterior


class GaussianLinearUniform(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("gaussian_linear_uniform", *args, **kwargs)


class GaussianMixture(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("gaussian_mixture", *args, **kwargs)


class LotkaVolterra(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("lotka_volterra", *args, **kwargs)

class LotkaVolterraJax(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("lotka_volterra_jax", *args, **kwargs)


class SIR(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("sir", *args, **kwargs)


class SLCP(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("slcp", *args, **kwargs)


class SLCPDistractors(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("slcp_distractors", *args, **kwargs)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(self.dataset).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation_distractors.csv"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.csv"))

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.csv"))

        if self.normalize:
            true_theta = (true_theta - self.mean) / self.std
            reference_posterior = (reference_posterior - self.mean) / self.std

        return observation, true_theta, reference_posterior


class TwoMoons(GeneratorDataloader):

    def __init__(self, *args, **kwargs):
        super().__init__("two_moons", *args, **kwargs)

# class Odisseo(GeneratorDataloader):

#     def __init__(self, *args, **kwargs,):
#         super().__init__("odisseo", *args, **kwargs, return_histogram=True)


class GeneratorDataloader_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        true_theta[:, 1:] = 10**true_theta[:, 1:]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_fix_position/sbi-sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 250_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        self.X[:, 1:] = 10**self.X[:, 1:]  # convert to original scale

        # if return_histogram == True:
        #     self.y = jnp.reshape(self.y, (-1, 5000, 6))
        #     print(f"Transforming {self.dataset} in histogram representation")
        #     self.transform_in_histogram()

        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        self.low=jnp.array([ 0.5,
                            10**3., 
                            10**log10(1/4 * 4.3683325e11), 
                            10**log10(1/4 *68_193_902_782.346756), ])
        self.high=jnp.array([5, 
                             10**4.5, 
                             10**log10(2 * 4.3683325e11),
                             10**log10(2 * 68_193_902_782.346756),])

        self.mean_y = self.y.mean(axis=0)
        self.std_y = self.y.std(axis=0)

        if self.normalize:
            pass
            # print(f"Normalizing {self.dataset} data")
            # self.X = (self.X - self.mean_X) / self.std_X
            self.X = 2 * (self.X - self.low)/(self.high - self.low) - 1
            # self.y = (self.y - self.mean_y) / self.std_y #this normalization is not needed for the histogram representation

        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class Odisseo(GeneratorDataloader_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo", *args, **kwargs, )


class GeneratorDataloaderAllParameters_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_fix_position/sbi-sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 250_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        self.low=jnp.array([0.5,
                            10**3., 
                            1/4 * 0.008,
                            10**log10(1/4 * 4.3683325e11), 
                            1/4 * 16,
                            10**log10(1/4 *68_193_902_782.346756),
                             1/4 * 3,
                              ])
        self.high=jnp.array([5, 
                             10**4.5, 
                             2 * 0.008, 
                             10**log10(2 * 4.3683325e11),
                             2 * 16,
                             10**log10(2 * 68_193_902_782.346756),
                             2 * 3,
                             ])

        self.mean_y = self.y.mean(axis=0)
        self.std_y = self.y.std(axis=0)

        if self.normalize:
            pass
            # print(f"Normalizing {self.dataset} data")
            # self.X = (self.X - self.mean_X) / self.std_X
            self.X = 2 * (self.X - self.low)/(self.high - self.low) - 1
            # self.y = (self.y - self.mean_y) / self.std_y #this normalization is not needed for the histogram representation

        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoAllParameters(GeneratorDataloaderAllParameters_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParameters", *args, **kwargs, )



class GeneratorDataloaderAllParametersOT_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_fix_position/sbi-sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 250_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        # self.mean_X = self.X.mean(axis=0)
        # self.std_X = self.X.std(axis=0)
        self.low=jnp.array([0.5,
                            10**3., 
                            1/4 * 0.008,
                            10**log10(1/4 * 4.3683325e11), 
                            1/4 * 16,
                            10**log10(1/4 *68_193_902_782.346756),
                             1/4 * 3,
                              ])
        self.high=jnp.array([5, 
                             10**4.5, 
                             2 * 0.008, 
                             10**log10(2 * 4.3683325e11),
                             2 * 16,
                             10**log10(2 * 68_193_902_782.346756),
                             2 * 3,
                             ])

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            uniform_01 = (self.X - self.low) / (self.high - self.low)
            # Convert to Gaussian space for fm_ot_loss
            self.X = norm.ppf(jnp.clip(uniform_01, 1e-6, 1-1e-6))
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParameters(GeneratorDataloaderAllParametersOT_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParameters", *args, **kwargs, )





class GeneratorDataloaderAllParameters_position_OT_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position/sbi-sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 250_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        # self.mean_X = self.X.mean(axis=0)
        # self.std_X = self.X.std(axis=0)
        self.low=jnp.array([0.5,
                            10**3., 
                            1/4 * 0.008,
                            10**log10(1/4 * 4.3683325e11), 
                            1/4 * 16,
                            10**log10(1/4 *68_193_902_782.346756),
                            1/4 * 3,
                            10.0, #x
                            0.1, #y
                            6.0, #z
                            90.0, #vx
                            -280.0, #vy
                            -120.0]) #vz
        self.high=jnp.array([5, 
                             10**4.5, 
                             2 * 0.008, 
                             10**log10(2 * 4.3683325e11),
                             2 * 16,
                             10**log10(2 * 68_193_902_782.346756),
                             2 * 3,
                             14.0, #x
                             2.5,  #y
                             8.0,  #z
                             115.0, #vx
                             -230.0, #vy
                             -80.0]) #vz

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            uniform_01 = (self.X - self.low) / (self.high - self.low)
            # Convert to Gaussian space for fm_ot_loss
            self.X = norm.ppf(jnp.clip(uniform_01, 1e-6, 1-1e-6))
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition(GeneratorDataloaderAllParameters_position_OT_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition", *args, **kwargs, )


class GeneratorDataloaderLOGAllParameters_position_OT_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position/sbi-sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 250_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        # self.mean_X = self.X.mean(axis=0)
        # self.std_X = self.X.std(axis=0)
        self.low=jnp.array([0.5,
                            3., 
                            1/4 * 0.008,
                            log10(1/4 * 4.3683325e11), 
                            1/4 * 16,
                            log10(1/4 *68_193_902_782.346756),
                            1/4 * 3,
                            10.0, #x
                            0.1, #y
                            6.0, #z
                            90.0, #vx
                            -280.0, #vy
                            -120.0]) #vz
        self.high=jnp.array([5, 
                             4.5, 
                             2 * 0.008, 
                             log10(2 * 4.3683325e11),
                             2 * 16,
                             log10(2 * 68_193_902_782.346756),
                             2 * 3,
                             14.0, #x
                             2.5,  #y
                             8.0,  #z
                             115.0, #vx
                             -230.0, #vy
                             -80.0]) #vz

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            uniform_01 = (self.X - self.low) / (self.high - self.low)
            # Convert to Gaussian space for fm_ot_loss
            self.X = norm.ppf(jnp.clip(uniform_01, 1e-6, 1-1e-6))
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTLOGAllParametersPosition(GeneratorDataloaderLOGAllParameters_position_OT_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition", *args, **kwargs, )


class GeneratorDataloaderAllParameters_position_OT_newprior_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_newprior/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 250_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_newprior(GeneratorDataloaderAllParameters_position_OT_newprior_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_newprior", *args, **kwargs, )



class GeneratorDataloaderAllParameters_position_error_OT_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    
    def add_noise(self, x,rng, noise_std=jnp.array([0.25, 0.001, 0.15, 5., 0.1, 1e-3])):
        x_noise = x + noise_std * jr.normal(key=rng, shape=(x.shape))
        return x_noise
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position/sbi-sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 250_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        # self.mean_X = self.X.mean(axis=0)
        # self.std_X = self.X.std(axis=0)
        self.low=jnp.array([0.5,
                            10**3., 
                            1/4 * 0.008,
                            10**log10(1/4 * 4.3683325e11), 
                            1/4 * 16,
                            10**log10(1/4 *68_193_902_782.346756),
                            1/4 * 3,
                            10.0, #x
                            0.1, #y
                            6.0, #z
                            90.0, #vx
                            -280.0, #vy
                            -120.0]) #vz
        self.high=jnp.array([5, 
                             10**4.5, 
                             2 * 0.008, 
                             10**log10(2 * 4.3683325e11),
                             2 * 16,
                             10**log10(2 * 68_193_902_782.346756),
                             2 * 3,
                             14.0, #x
                             2.5,  #y
                             8.0,  #z
                             115.0, #vx
                             -230.0, #vy
                             -80.0]) #vz

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            uniform_01 = (self.X - self.low) / (self.high - self.low)
            # Convert to Gaussian space for fm_ot_loss
            self.X = norm.ppf(jnp.clip(uniform_01, 1e-6, 1-1e-6))
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]
        

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y =  self.add_noise(x=y, rng=jr.PRNGKey(self.current_step))
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y =  self.add_noise(x=y, rng=jr.PRNGKey(self.current_step))
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_error(GeneratorDataloaderAllParameters_position_error_OT_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition", *args, **kwargs, )



class GeneratorDataloaderAllParameters_position_OT_uniformprior_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_uniform_prior/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 200_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_uniformprior(GeneratorDataloaderAllParameters_position_OT_uniformprior_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_uniformprior", *args, **kwargs, )



class GeneratorDataloaderAllParameters_position_OT_uniformprior_error_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    
    def add_noise(self, x,rng, noise_std=jnp.array([0.25, 0.001, 0.15, 5., 0.1, 1e-3])):
        x_noise = x + noise_std * jr.normal(key=rng, shape=(x.shape))
        return x_noise
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_uniform_prior/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 200_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:
            self.rng = jr.split(self.rng)[0]

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y =  self.add_noise(x=y, rng=self.rng)
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y =  self.add_noise(x=y, rng=self.rng)
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_uniformprior_error(GeneratorDataloaderAllParameters_position_OT_uniformprior_error_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_uniformprior", *args, **kwargs, )



class GeneratorDataloaderAllParameters_position_OT_uniformprior_30e5_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_3e5_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_uniform_prior/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [300_000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [300_000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_uniformprior_30e5(GeneratorDataloaderAllParameters_position_OT_uniformprior_30e5_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_uniformprior", *args, **kwargs, )


class GeneratorDataloaderAllParameters_position_OT_uniformprior_30e5_uniformnorm_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_3e5_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_uniform_prior/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [300_000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [300_000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        # self.mean_X = self.X.mean(axis=0)
        # self.std_X = self.X.std(axis=0)
        self.low=jnp.array([0.5,
                            10**3., 
                            1/2 * 0.008,
                            log10(1/2 * 4.3683325e11), 
                            1/2 * 16,
                            log10(1/2 * 68_193_902_782.346756),
                            1/2 * 3,
                            10.0, #x
                            0.1, #y
                            6.0, #z
                            90.0, #vx
                            -280.0, #vy
                            -120.0]) #vz
        self.high=jnp.array([5, 
                             10**4.5, 
                             2 * 0.008, 
                             10**log10(2 * 4.3683325e11),
                             2 * 16,
                             10**log10(2 * 68_193_902_782.346756),
                             2 * 3,
                             14.0, #x
                             2.5,  #y
                             8.0,  #z
                             115.0, #vx
                             -230.0, #vy
                             -80.0]) #vz
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            uniform_01 = (self.X - self.low) / (self.high - self.low)
            self.X = 2 * uniform_01 - 1 
 
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_uniformprior_30e5_uniformnorm(GeneratorDataloaderAllParameters_position_OT_uniformprior_30e5_uniformnorm_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_uniformprior", *args, **kwargs, )

class GeneratorDataloaderAllParameters_position_OT_uniformprior_log_30e5_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_3e5_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_uniform_prior/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [300_000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [300_000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale
        self.X[:, [0, 1, 2, 4, 6]] = np.log10(self.X[:, [0, 1, 2, 4, 6]])


        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_uniformprior_log_30e5(GeneratorDataloaderAllParameters_position_OT_uniformprior_log_30e5_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_uniformprior", *args, **kwargs, )



class GeneratorDataloaderAllParameters_position_OT_uniformprior_TSIT5_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_uniform_prior_TSIT5/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 200_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_uniformprior_TSIT5(GeneratorDataloaderAllParameters_position_OT_uniformprior_TSIT5_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_uniformprior_TSIT5", *args, **kwargs, )




class GeneratorDataloaderAllParameters_position_OT_uniformprior_TSIT5_error_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def add_noise(self, x,rng, noise_std=jnp.array([0.25, 0.001, 0.15, 5., 0.1, 0.0001])):
        x_noise = x + noise_std * jr.normal(key=rng, shape=(x.shape))
        return x_noise

    def load_file(self, file):

        return np.load(file, allow_pickle=True)

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"))
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_uniform_prior_TSIT5/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 200_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"))
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y =  self.add_noise(x=y, rng=self.rng)
                
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y =  self.add_noise(x=y, rng=self.rng)
                self.rng = jr.split(self.rng)[0]

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_uniformprior_TSIT5_error(GeneratorDataloaderAllParameters_position_OT_uniformprior_TSIT5_error_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParametersPosition_uniformprior_TSIT5", *args, **kwargs, )



class GeneratorDataloaderAllParameters_fixposition_OT_uniformprior_TSIT5_error_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def add_noise(self, x,rng, noise_std=jnp.array([0.25, 0.001, 0.15, 5., 0.1, 0.0001])):
        x_noise = x + noise_std * jr.normal(key=rng, shape=(x.shape))
        return x_noise

    def load_file(self, file, use_log=True):

        if use_log:
            return np.load(file, allow_pickle=True)
        else:
            return 10**(np.load(file, allow_pickle=True))

    def get_observation(self, idx, ):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"), use_log=self.use_log)
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"), )

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, noise: bool = True, use_log: bool = True):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_fix_position_uniform_prior_TSTIT5/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 200_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)
        self.use_log = use_log
        self.noise = noise

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"),)
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"),  use_log=use_log)
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                # y =  self.add_noise(x=y, rng=self.rng)
                
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                # y =  self.add_noise(x=y, rng=self.rng)
                self.rng = jr.split(self.rng)[0]
            
            if self.noise:
                y =  self.add_noise(x=y, rng=self.rng)

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersFiXPosition_uniformprior_TSIT5_error(GeneratorDataloaderAllParameters_fixposition_OT_uniformprior_TSIT5_error_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("odisseo_AllParameters_fixposition_uniformprior_TSIT5", *args, **kwargs, )



class GeneratorDataloaderAllParametersPositions_fixedtime_OT_uniformprior_TSIT5_error_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def add_noise(self, x,rng, noise_std=jnp.array([0.25, 0.001, 0.15, 5., 0.1, 0.0001])):
        x_noise = x + noise_std * jr.normal(key=rng, shape=(x.shape))
        return x_noise

    def load_file(self, file, use_log: bool = True):

        data = np.load(file, allow_pickle=True)
        if use_log:
            return data
        else:
            data[:, [0, 1, 2, 4, 6]] = 10**data[:, [0, 1, 2, 4, 6]]
            return data

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"), use_log=self.use_log)
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False, noise: bool = True, use_log: bool = True):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/odisseo_data/data_varying_position_fixed_time_uniform_prior_TSTIT5/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 200_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)
        self.use_log = use_log
        self.noise = noise

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"), use_log=self.use_log)
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                
                
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]
            
            if self.noise:
                y =  self.add_noise(x=y, rng=self.rng)

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class OdisseoOTAllParametersPosition_fixedtime_uniformprior_TSIT5_error(GeneratorDataloaderAllParametersPositions_fixedtime_OT_uniformprior_TSIT5_error_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("data_varying_position_fixed_time_uniform_prior_TSTIT5", *args, **kwargs, )



class GeneratorDataloaderAllParametersPositions_galax_OT_uniformprior_numpy(Iterable):
    num_samples: int
    rng: jr.PRNGKey
    num_steps: int
    batch_size: int
    X: jnp.ndarray
    y: jnp.ndarray
    normalize: bool

    def __iter__(self):
        self.current_step = 1

        if not self.replacement:
            perm = jr.permutation(self.rng, self.X.shape[0])
            self.X = self.X[perm]
            self.y = self.y[perm]
            self.rng = jr.split(self.rng)[0]

        return self

    def add_noise(self, x,rng, noise_std=jnp.array([0.25, 0.001, 0.15, 5., 0.1, 0.0001])):
        x_noise = x + noise_std * jr.normal(key=rng, shape=(x.shape))
        return x_noise

    def load_file(self, file, use_log: bool = True):

        data = np.load(file, allow_pickle=True)
        if use_log:
            return data
        else:
            data[:, [0, 1, 2, 4, 6]] = 10**data[:, [0, 1, 2, 4, 6]]
            return data

    def get_observation(self, idx):

        base_dir = Path(self.DATA_ROOT).joinpath(f'num_observation_{idx}')

        if not base_dir.exists():
            raise FileNotFoundError(f"Directory {base_dir} does not exist")

        observation = self.load_file(base_dir.joinpath(f"observation.npy"))

        true_theta = self.load_file(base_dir.joinpath(f"true_parameters.npy"), use_log=self.use_log)
        # true_theta[:, [1, 3, 5]] = 10**true_theta[:,[1, 3, 5]]  # convert to original scale

        reference_posterior = self.load_file(base_dir.joinpath(f"reference_posterior_samples.npy"))

        # if self.normalize:
        #     # true_theta = (true_theta - self.mean_X) / self.std_X
        #     # reference_posterior = (reference_posterior - self.mean_X) / self.std_X
        #     # observation = (observation - self.mean_y) / self.std_y
        #     # Normalize each histogram to sum to 1 (convert to probability distribution)
        #     total = jnp.sum(observation, axis=(1, 2), keepdims=True)
        #     observation = observation / (total + 1e-8)  # Add epsilon to avoid division by zero

        return observation, true_theta, reference_posterior
    

    def __init__(self, dataset: str, DATA_ROOT: str, num_samples: int, split: Tuple[int], seed: jr.PRNGKey,
                 num_steps: int, batch_size: int, normalize: bool = True, replacement: bool = False, return_histogram: bool = False, noise: bool = True, use_log: bool = True):
        """
        :param dataset:
        :param DATA_ROOT:
        :param num_samples:
        :param split:
        :param seed:
        :param num_steps:
        :param batch_size:
        :param normalize:
        :param replacement:
        """
        print(dataset)

        DATA_ROOT = f'/export/data/vgiusepp/galax_data/data_varying_position_uniform_prior/sbi_sim/data/sbi-benchmarks/{dataset}/'
        print(f"DATA_ROOT: {DATA_ROOT}")
        path = Path(DATA_ROOT)
        # path = path.joinpath(dataset)

        if num_samples not in [100, 1000, 10000, 100_000, 200_000, 1_000_000, 10000000]:
            raise ValueError(f"num_samples ({num_samples}) must be in [100, 1000, 10000, 100000, 250_000, 1000000]")

        self.replacement = replacement

        self.normalize = normalize
        self.DATA_ROOT = DATA_ROOT
        self.dataset = dataset
        self.batch_size = batch_size
        self.rng = jr.PRNGKey(seed)
        self.use_log = use_log
        self.noise = noise

        self.current_step = 1
        self.dim_theta = 10

        split_start = split[0]
        split_end = split[1]

        self.load_data_from_hf(dataset, Path(DATA_ROOT))

        self.y = self.load_file(path.joinpath(f"x_{num_samples}.npy"))
        self.X = self.load_file(path.joinpath(f"theta_{num_samples}.npy"), use_log=self.use_log)
        # self.X[:, [1, 3, 5]] = 10**self.X[:, [1, 3, 5]]  # convert to original scale



        # samples seem to be sorted when loading from file; make sure to shuffle them!
        permutation_seed = jr.PRNGKey(0)
        perm = jr.permutation(permutation_seed, num_samples)
        self.X = self.X[perm]
        self.y = self.y[perm]

        self.mean_X = self.X.mean(axis=0)
        self.std_X = self.X.std(axis=0)
        

        # self.mean_y = self.y.mean(axis=0)
        # self.std_y = self.y.std(axis=0)

        if self.normalize:
            # Convert uniform to [0,1] first
            self.X = (self.X - self.mean_X) / self.std_X
            
        self.X = self.X[split_start:split_end]
        self.y = self.y[split_start:split_end]

        self.num_samples = self.X.shape[0]
        if not self.replacement:
            self.num_steps = int(ceil(self.num_samples / self.batch_size))
            print(f"Dataloader sampling without replacements (num_steps: {self.num_steps})")
        else:
            self.num_steps = num_steps

    def __next__(self):
        if self.current_step <= self.num_steps:

            if not self.replacement:
                X = jnp.array(self.X[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                y = jnp.array(self.y[self.batch_size * (self.current_step-1): self.batch_size * self.current_step])
                
            else:
                X = jnp.array(self.X[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                y = jnp.array(self.y[jr.choice(self.rng, self.num_samples, (self.batch_size,))])
                self.rng = jr.split(self.rng)[0]
            
            if self.noise:
                y =  self.add_noise(x=y, rng=self.rng)

            self.current_step += 1
            return {'parameters': X, 'weighting': jnp.ones((self.batch_size, 1)), 'conditioning': y}

        else:

            raise StopIteration

    def __len__(self):
        return self.num_steps
    
    def load_data_from_hf(self, dataset, path):

        path_dir = Path(path)
        if not path_dir.exists():

            # download folder from huggingface
            from huggingface_hub import hf_hub_download, snapshot_download
            print(f"Downloading dataset {dataset} from huggingface...")

            snapshot_download(repo_id=f"thuerey-group/sbi-toy-datasets", 
                          repo_type="dataset",
                          local_dir=path,
                          allow_patterns=[f"{dataset}/*"])
            
        else:

            print(f"Loading {dataset} locally from existing directory {path_dir}")


class galaxOTAllParametersPosition_uniformprior(GeneratorDataloaderAllParametersPositions_galax_OT_uniformprior_numpy):

    def __init__(self, *args, **kwargs,):
        super().__init__("galax_AllParametersPosition_uniformprior", *args, **kwargs, )