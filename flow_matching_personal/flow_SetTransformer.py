...
class Flow(nn.Module,  FlaxModelMixin, ConfigMixin):
    ...
    dim_flow: int = 7  #must be set for the output dimension, like a final MLP output
    N_dim: int = 64
    N_head: int = 8
    depth: int = 4
    N_seed: int = 1
    ...
    def init_weights(self, rng: jax.Array) -> FrozenDict:
            ...
            return self.init(rngs, encoder_hidden_states=sample, timesteps=timesteps, sample=encoder_hidden_states, )["params"]
    ...
    def __call__(
        ...
        print("encoder_hidden_states after norm", encoder_hidden_states.shape)
