...
# Linear-space prior bounds (your original numbers)
prior_bounds_linear = jnp.array([
    [0.5, 5.0],                    # t_end (linear)
...
log_mask = jnp.array([False, True, True, True, True, True, False])
...
samples = np.savez(f"mcmc_numpyro_samples_{master_key_int}.npz", samples_log=samples)

# raw expected shape: (num_chains, num_samples, D)
print("raw sample shape:", np.array(raw).shape)

# Combine chains and samples
raw_flat = jnp.reshape(raw, (-1, D))  # (num_chains*num_samples, D)

# Convert back to linear space for interpretation
@jit
def transform_samples_log_to_lin(samples_log):
    # samples_log shape: (N, D)
    return jnp.where(log_mask, jnp.exp(samples_log), samples_log)

samples_lin = transform_samples_log_to_lin(raw_flat)
samples_np = np.array(samples_lin)  # convert to numpy for DataFrame

samples_df = pd.DataFrame(samples_np, columns=list(param_names))
